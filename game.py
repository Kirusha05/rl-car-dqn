...
class CarEnv:
    def __init__(self):
        self.velocity = 3
        self.turn_speed = math.pi / 15 # 6 deg
        self.car_width = 30
        self.car_height = 10
        self.track = Track()
        ...
        # state consisting of 5 sensors data and 3 possible actions
        self.observation_space_n = 5
        self.action_space = ActionSpace(actions=[0, 1, 2])
    ...
    def reset(self):
        ...
        _, sensors = self._get_observations()
        # return state
        return sensors

    def step(self, action: Literal[0, 1, 2], truncate_at=3000):
        self.steps += 1
        ...
        # 3. Get observations
        car_is_on_track, sensors = self._get_observations()
        ...
        return sensors, reward, self.done, truncated, {}
    
    def _apply_action(self, action: Literal[0, 1, 2]):
        if action == 0:
            self._steer_left() # go left
            self.step_data["left"] += 1
        elif action == 1:
            self._steer_right() # go right
            self.step_data["right"] += 1
        elif action == 2:
            self.step_data["straight"] += 1
    ...
    def _get_observations(self):
        ...
        return car_is_on_track, sensors
    ...
    def _compute_reward(self, car_is_on_track, sensors) -> (int, bool):
        ...
        left_dist, _, _, _, right_dist = sensors
        diff = abs(left_dist - right_dist) * 10
        complement = 1 - diff  # the greater the diff, the smaller the reward
        return 5 * complement, False
    ...
    def render(self):
        ...
        for event in pygame.event.get():
            ...
            #     self.track.register_track_points(event)

        # If you want to sabotage the agent
        ...
        text_surface = self.font.render(f"Left: {self.step_data["left"]}", True, (255, 255, 255))
        self.screen.blit(text_surface, (left, top + 30))
        text_surface = self.font.render(f"Straight: {self.step_data["straight"]}", True, (255, 255, 255))
        self.screen.blit(text_surface, (left, top + 60))
        text_surface = self.font.render(f"Right: {self.step_data["right"]}", True, (255, 255, 255))
        self.screen.blit(text_surface, (left, top + 90))
